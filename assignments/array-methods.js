...
let ticketPriceTotal = runners.reduce((acc, val) => { 
    ...
    },0) // Starts at 0 and adds each itteration 
...
// Problem 1 Runners who donated over 100 get a special gift. Print all runners that donated over 100. 

let ballers = runners.filter(function (runner){ 
    return runner.donation >100
    })
    
    console.log(ballers)

// Problem 2 We have a marketing email and need to send all runners an email. Print out the email of all runners. 

let contact = runners.map(function(runner){ 
    return runner.email
})
console.log(contact)

// Problem 3 Print of list of each runners company name. 

let companies = [];
runners.forEach(function(a){
    companies.push(a.company_name);
  })
